    ...
    <div class="mb-4 space-y-4">
        ...
        <div id="camera-container" class="{{ $cameraActive ? 'block' : 'hidden' }} relative">
            ...
            <button 
                type="button"
                id="capture-btn"
                class="absolute bottom-4 px-4 py-2 text-sm font-medium bg-red-600 rounded-full hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500"
            >
                Capture
            </button>
        </div>
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            @this.on('photo-captured', (data) => {
                ...
                const hiddenInput = document.querySelector('input[name="photo_path"]');
                if (hiddenInput) {
                    hiddenInput.value = data.path;
                    // Dispatch an event to trigger Filament's data binding
